...
class BrowserSession:
    ...
    async def init(self):
        ...
        for attempt in range(max_retries):
            try:
                logger.debug(f"Attempting to connect to existing browser instance via CDP... (attempt {attempt + 1}/{max_retries})")
                ...
                logger.info("Connected to Electron browser instance")
                break
            except asyncio.TimeoutError:
                logger.debug(f"Connection attempt {attempt + 1} timed out after 3 seconds")
                if attempt + 1 >= max_retries:
                    logger.info("All connection attempts failed, launching new browser instance")
                    ...
                    logger.debug(f"Retrying in 0.5 seconds...")
    ...
    async def _find_tagged_main_page(self):
            ...
            for page in context.pages:
                try:
                    ...
                    if is_main_app:
                        ...
                        page_url = page.url
                        logger.info(f"Found tagged main app page: '{page_title}' - {page_url}")
                        page_found = True
                        break
                    else:
                        ...
                        try:
                            page_title = await asyncio.wait_for(page.title(), timeout=1.0)
                            page_url = page.url
                            logger.debug(f"[CDP] Found page (not main): '{page_title}' - {page_url}")
                        except Exception:
                            pass
                            
                except asyncio.TimeoutError:
                    logger.debug(f"Timeout checking page for tag")
                    continue
                except Exception as e:
                    logger.debug(f"Error checking page for tag: {e}")
                    continue
        ...
        if not page_found:
            logger.info("No tagged main app page found")
            ...
            for context in self.browser.contexts:
                for page in context.pages:
                    try:
                        ...
                        logger.debug(f"  - {tag_status} '{page_title}' - {page_url}")
                    except Exception as e:
    ...
    async def _fallback_find_and_tag_page(self):
        ...
        logger.debug("🔄 Fallback: Looking for suitable page to tag...")
            ...
            for page in context.pages:
                try:
                    ...
                    if (page_url and 
                        ...
                        logger.info(f"🎯 Found suitable page to tag: '{page_title}' - {page_url}")
                        self.page = page
                        await self._tag_page_as_main_app()
                        return
                        
                except Exception as e:
                    logger.debug(f"Error checking page in fallback: {e}")
                    continue
        
        # If no suitable page found, create a new one
        logger.info("No suitable page found, creating new page")
        self.page = await self.browser.new_page()
        await self._tag_page_as_main_app()
        logger.debug("Created and tagged new page")
